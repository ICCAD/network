...
int main(int argc, char** argv)
{
	...
	for( int i=0;i<101;i++ ){
		for( int j=1;j<101;j+=2 ){
			network[101*i+j] = 1;
			flow_rate[101*i+j] = 0.1 * pow(10,11);
			if(j < 50){
				direction[101*i+j] = 4;
			}
			else{
	...
	/*for( int i=0;i<80;i++ ){
		network[101*i+50] = 1;
		direction[101*i+50] = 4;
		flow_rate[101*i+50] = 7 * pow(10,11);
		
	}
	for( int i=0;i<50;i++ ){
		network[101*79+i] = 1;
		direction[101*79+i] = 3;
		flow_rate[101*79+i] = 3.5 * pow(10,11);
		
	}
	for( int i=51;i<101;i++ ){
		network[101*79+i] = 1;
		direction[101*79+i] = 1;
		flow_rate[101*79+i] = 3.5 * pow(10,11);
		
	}
	network[50] = 2;
	network[101*79] = 3;
	network[101*79+100] = 3;*/
	
	/*for( int i=0;i<101;i++ ){
		for( int j=0;j<101;j++ ){
			fout << network[101*i+j] << " " ;
		}
		fout << endl;
	}*/
    ...
    SimResult_t (*emulate) (ThermalData_t*, Dimensions_t*, Analysis_t*, int**, double**) ;
    ...
    error = thermal_data_build
        ...
        (&tdata, &stkd.StackElements, stkd.Dimensions, &analysis, network, flow_rate, direction) ;
    ...
    do
    {
        sim_result = emulate (&tdata, stkd.Dimensions, &analysis, network, flow_rate) ;
