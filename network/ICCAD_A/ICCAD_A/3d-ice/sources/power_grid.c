        ...
        switch (pgrid->LayersProfile [layer])
        {
            ...
            case TDICE_LAYER_PINFINS_STAGGERED :
            {
				...
				FOR_EVERY_ROW (row, dimensions){
					FOR_EVERY_COLUMN (column, dimensions)
					{
						if (network[layer/3][row*101+column] == 2)
						{
							*tmp =   (Source_t) 2.0

								   * get_convective_term_2

										(pgrid->Channel->Coolant.VHC, flow_rate[layer/3][row*101+column])
...
void update_channel_sources (PowerGrid_t *pgrid, Dimensions_t *dimensions, double **flow_rate)
{
    Quantity_t   layer ;
    Source_t    *sources ;
                    ...
                    if (IS_CHANNEL_COLUMN (pgrid->Channel->ChannelModel, column) == true)
                    {
                        *tmp =   (Source_t) 2.0
                               ...
                               * get_convective_term_2
                                    ...
                                    (pgrid->Channel->Coolant.VHC, flow_rate[layer/3][column])
