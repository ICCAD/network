...
{
    Conductance_t __attribute__ ((unused)) conductance = 0.0 ;
	Conductance_t __attribute__ ((unused)) conductance_2 = 0.0 ;
    ...
    *sysmatrix.RowIndices++ = get_cell_offset_in_stack
	...
	g_bottom = get_conductance_bottom_2

        (thermal_grid, dimensions, layer_index    , row_index, column_index, network) ;

    g_top = get_conductance_top_2

        (thermal_grid, dimensions, layer_index - 1, row_index, column_index, network) ;

    if (g_top == 0)

        conductance_2 = g_bottom ;

    else

        conductance_2 = PARALLEL (g_bottom, g_top) ;
...
skip_bottom :
    ...
    *sysmatrix.RowIndices++ = get_cell_offset_in_stack
	...
	g_south = get_conductance_south_2

        (thermal_grid, dimensions, layer_index, row_index    , column_index, network) ;

    g_north = get_conductance_north_2

        (thermal_grid, dimensions, layer_index, row_index - 1, column_index, network) ;

    conductance_2 = PARALLEL (g_south, g_north) ;
...
skip_south :
    ...
    *sysmatrix.RowIndices++ = get_cell_offset_in_stack
	...
	g_west = get_conductance_west_2

        (thermal_grid, dimensions, layer_index, row_index, column_index    , network) ;

    g_east = get_conductance_east_2

        (thermal_grid, dimensions, layer_index, row_index, column_index - 1, network) ;

    conductance_2 = PARALLEL (g_west, g_east) ;
...
skip_west :
    ...
    *sysmatrix.Values = 0.0 ;
    ...
    {
        *sysmatrix.Values = get_capacity_2
    ...
    if (   thermal_grid->LayersProfile [layer_index] == TDICE_LAYER_SINK
        ...
        *sysmatrix.Values += get_conductance_top_2
    ...
    *sysmatrix.RowIndices++ = get_cell_offset_in_stack
    ...
    conductance_2 = PARALLEL (g_east, g_west) ;
...
skip_east :
    ...
    *sysmatrix.RowIndices++ = get_cell_offset_in_stack
    ...
    conductance_2 = PARALLEL (g_north, g_south) ;
...
skip_north :
    ...
    *sysmatrix.RowIndices++ = get_cell_offset_in_stack
    ...
    if (g_bottom == 0)
        ...
        conductance_2 = PARALLEL (g_top, g_bottom) ;
    ...
    *sysmatrix.ColumnPointers = *(sysmatrix.ColumnPointers - 1) ;
    ...
    if ( ! IS_FIRST_LAYER(layer_index) )
    {
        *sysmatrix.RowIndices++ = get_cell_offset_in_stack
		...
		conductance_2 = PARALLEL
        (
            get_conductance_bottom_2 (thermal_grid, dimensions, layer_index    , row_index, column_index, network),
            get_conductance_top_2    (thermal_grid, dimensions, layer_index - 1, row_index, column_index, network)
        ) ;
    ...
    if ( ! IS_FIRST_ROW(row_index) )
    {
        *sysmatrix.RowIndices++ = get_cell_offset_in_stack
        ...
        conductance = get_conductance_north /* == C */

            (thermal_grid, dimensions, layer_index, row_index, column_index) ;
		
		if(network[layer_index/3][(row_index-1)*101+column_index]){
			
			if(network[layer_index/3][row_index*101+column_index] == 4){
				conductance_2 = get_inv(2,direction[layer_index/3][(row_index-1)*101+column_index])
			...
			*sysmatrix.Values++ = conductance_2 ;
			...
			conductance_2 = PARALLEL
			(
				get_conductance_south_2    (thermal_grid, dimensions, layer_index , row_index    , column_index, network),
				get_conductance_north_2    (thermal_grid, dimensions, layer_index , row_index - 1, column_index, network)
			) ;
			diagonal_value +=  conductance_2 ;
			*sysmatrix.Values++ = -conductance_2 ;
		...
		if(network[layer_index/3][row_index*101+column_index-1]){
			...
			if(network[layer_index/3][row_index*101+column_index] == 4){
				conductance_2 = get_inv(4,direction[layer_index/3][row_index*101+column_index-1])
								* get_convective_term_2(thermal_grid->Channel->Coolant.VHC,flow_rate[layer_index/3][row_index*101+column_index-1]) ;
			}
			else{
				conductance_2 = get_inv(4,direction[layer_index/3][row_index*101+column_index])
								* get_convective_term_2(thermal_grid->Channel->Coolant.VHC,flow_rate[layer_index/3][row_index*101+column_index]) ;
			}
			*sysmatrix.Values++ = conductance_2 ;
			check += conductance_2;
			...
			conductance_2 = PARALLEL
			(
				get_conductance_west_2 (thermal_grid, dimensions, layer_index, row_index, column_index    ,network),
				get_conductance_east_2 (thermal_grid, dimensions, layer_index, row_index, column_index - 1,network)
			) ;
			diagonal_value +=  conductance_2 ;
			*sysmatrix.Values++ = -conductance_2 ;
    ...
    *sysmatrix.Values = 0.0 ;
    ...
    if ( ! IS_LAST_COLUMN(column_index, dimensions) )
    {
        *sysmatrix.RowIndices++ = get_cell_offset_in_stack
		...
		if(network[layer_index/3][row_index*101+column_index+1]){
			...
			if(network[layer_index/3][row_index*101+column_index] == 4){
				conductance_2 = get_inv(1,direction[layer_index/3][row_index*101+column_index+1])
								* get_convective_term_2(thermal_grid->Channel->Coolant.VHC,flow_rate[layer_index/3][row_index*101+column_index+1]) ;
			}
			else{
				conductance_2 = get_inv(1,direction[layer_index/3][row_index*101+column_index])
								* get_convective_term_2(thermal_grid->Channel->Coolant.VHC,flow_rate[layer_index/3][row_index*101+column_index]) ;
			}
			*sysmatrix.Values++ = conductance_2 ;
			check += conductance_2;
			...
			conductance_2 = PARALLEL
			(
				get_conductance_east_2 (thermal_grid, dimensions, layer_index, row_index, column_index    ,network),
				get_conductance_west_2 (thermal_grid, dimensions, layer_index, row_index, column_index + 1,network)
			) ;
			diagonal_value +=  conductance_2 ;
			*sysmatrix.Values++ = -conductance_2 ;
    ...
    if ( ! IS_LAST_ROW(row_index, dimensions) )
    {
        *sysmatrix.RowIndices++ = get_cell_offset_in_stack
         ...
         conductance = get_conductance_south /* == - C */
		...
		if(network[layer_index/3][(row_index+1)*101+column_index]){
			...
			*sysmatrix.Values++ = conductance_2 ;
			...
			conductance_2 = PARALLEL
			(
				get_conductance_north_2    (thermal_grid, dimensions, layer_index , row_index    , column_index, network),
				get_conductance_south_2    (thermal_grid, dimensions, layer_index , row_index + 1, column_index, network)
			) ;
			diagonal_value +=  conductance_2 ;
			*sysmatrix.Values++ = -conductance_2 ;
    ...
    if ( ! IS_LAST_LAYER(layer_index, dimensions) )
    {
        *sysmatrix.RowIndices++ = get_cell_offset_in_stack
		...
		conductance_2 = PARALLEL
        (
            get_conductance_top_2    (thermal_grid, dimensions, layer_index    , row_index, column_index, network),
            get_conductance_bottom_2 (thermal_grid, dimensions, layer_index + 1, row_index, column_index, network)
        ) ;
    ...
    *diagonal_pointer += diagonal_value ;
    ...
    if ((IS_FIRST_ROW(row_index) || IS_LAST_ROW(row_index, dimensions) || IS_FIRST_COLUMN(column_index) || IS_LAST_COLUMN(column_index, dimensions))
        ...
        *diagonal_pointer += get_convective_term_2 /* == C */
            ...
            (thermal_grid->Channel->Coolant.VHC, flow_rate[layer_index/3][row_index*101+column_index]) ;
        ...
        switch (thermal_grid->LayersProfile [lindex])
        {
                    ...
                    FOR_EVERY_COLUMN (column_index, dimensions)
                    {
                        tmp_matrix = add_solid_column
                            ...
                            (tmp_matrix, thermal_grid, analysis, dimensions,
                             lindex, row_index, column_index, network) ;
            ...
            case TDICE_LAYER_CHANNEL_4RM :
            {
                FOR_EVERY_ROW (row_index, dimensions)
                {
                    FOR_EVERY_COLUMN (column_index, dimensions)
                    {
                        if (network[lindex/3][row_index*101+column_index])

                            tmp_matrix = add_liquid_column_4rm

                                (tmp_matrix, thermal_grid, analysis, dimensions,
                                 lindex, row_index, column_index, network, flow_rate, direction) ;
