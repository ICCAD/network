...
)
{
    ...
    fill_system_matrix
        ...
        (&tdata->SM_A, &tdata->ThermalGrid, analysis, dimensions, network, flow_rate, direction) ;
...
(
    ...
    Dimensions_t   *dimensions,
    Analysis_t     *analysis,
	int __attribute__ ((unused)) **network,
	double __attribute__ ((unused)) **flow_rate
)
{
    ...
    if (slot_completed (analysis) == true)
    {
        Error_t result = update_source_vector
            ...
            (&tdata->PowerGrid, &tdata->ThermalGrid, dimensions, network, flow_rate) ;
...
)
{
    ...
    do
    {
        ...
        result = emulate_step (tdata, dimensions, analysis, network, flow_rate) ;
...
)
{
    ...
    fill_system_matrix (&tdata->SM_A, &tdata->ThermalGrid, analysis, dimensions, network, flow_rate, direction) ;

    if (do_factorization (&tdata->SM_A) == TDICE_FAILURE)

        return TDICE_FAILURE ;

    update_channel_sources (&tdata->PowerGrid, dimensions, flow_rate) ;
