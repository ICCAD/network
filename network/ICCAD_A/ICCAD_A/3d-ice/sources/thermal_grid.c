    ...
    switch (tgrid->LayersProfile [layer_index])
    {
        ...
        case TDICE_LAYER_CHANNEL_4RM :
            ...
            if (IS_CHANNEL_COLUMN (TDICE_CHANNEL_MODEL_MC_4RM, column_index)){
				...
				//cout << setprecision(15) << tgrid->Channel->Coolant.VHC << endl;
				//getchar();
			}
            else
...
Conductance_t get_conductance_top_2
(
    ThermalGrid_t *tgrid,
    Dimensions_t  *dimensions,
    CellIndex_t    layer_index,
...
Conductance_t get_conductance_bottom_2
(
    ThermalGrid_t *tgrid,
    Dimensions_t  *dimensions,
    CellIndex_t    layer_index,
...
Conductance_t get_conductance_north_2
(
    ThermalGrid_t *tgrid,
    Dimensions_t  *dimensions,
    CellIndex_t    layer_index,
...
Conductance_t get_conductance_south_2
(
    ThermalGrid_t *tgrid,
    Dimensions_t  *dimensions,
    CellIndex_t    layer_index,
...
Conductance_t get_conductance_east_2
(
    ThermalGrid_t *tgrid,
    Dimensions_t  *dimensions,
    CellIndex_t    layer_index,
...
Conductance_t get_conductance_west_2
(
    ThermalGrid_t *tgrid,
    Dimensions_t  *dimensions,
    CellIndex_t    layer_index,
...
double get_HTC(double w, double h){
	double dh = 2 * w * h / (w + h) ;
	double r = w / h;
	double Nu = 8.235 - (1 - 2.0421 * pow(r,1) + 3.0853 * pow(r,2) - 2.4765 * pow(r,3) + 1.0578 * pow(r,4) - 0.1861 * pow(r,5)) ;
	return 0.6 * Nu / dh ;
}

double get_convective_term_2(double VHC,double flow_rate){
	return VHC * flow_rate / 2 ;
}

double get_inv(int dir_a,int dir_b){
	int data = dir_a & dir_b;
	if(data != 0){
		return -1;
	}
	else{
